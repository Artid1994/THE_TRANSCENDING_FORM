...
class MemoryGraph:
    ...
    def co_activate(self, source: str, target: str) -> tuple[str, str]:
        ...
        edge = self.connect(source, target)
        self.active_edges.add((source, target))
    ...
    def snapshot_activity(self) -> dict[str, list]:
        """Return pending activity without consuming it."""
        return {
            "nodes": sorted(self.active_nodes),
            "edges": [
                [source, target]
                for source, target in sorted(self.active_edges)
            ],
        }

    def consume_activity(self) -> dict[str, list]:
        """Return and clear activity generated by the real MemoryGraph."""
        activity = {
            "nodes": sorted(self.active_nodes),
            "edges": [
                [source, target]
